 ...
 */
Carton::Carton(double side1, double side2, double side3) : ShippingItem(){
  try
  {
...
/**
 * @brief - This is a second non-default constructor that also takes an address structure when setting itself up.
 * @param address - a structure of strings about the shippers address.
 * @param weight - a double for the weight of the shipping item.
 * @param side1 - one dimension of the carton
 * @param side2 - one dimension of the carton
 * @param side3 - one dimension of the carton
 */
...
void Carton::set_measurements(double side1, double side2, double side3) {
  ...
  if (side1 >= side2 && side1 >= side3) {
    shipping_length_ = side1;
    width_ = side2;
    height_ = side3;
  } else if (side2 >= side1 && side2 >= side3) {
    shipping_length_ = side2;
    width_ = side3;
    height_ = side1;
  } else {
    shipping_length_ = side3;
    width_ = side1;
    height_ = side2;
  }
}
 ...
 */
double Carton::Volume() const { return shipping_length_ * width_ * height_; }
 ...
 */
double Carton::LengthPlusGirth() const { return shipping_length_ + Girth(); }
...
bool Carton::SizedToShip() const {
  // check if too small
  if (shipping_length_ < kMinLength) {
    return false;
  }
  if (height_ <= width_) {
...
void Carton::WriteData(std::ostream& out) const {
  out << shipping_length_ << " " << width_ << " " << height_ << endl;
}
/**
...
void Carton::WriteCarton(std::ostream& out) const {
  out << "A carton with length " << shipping_length_ << ", width " << width_
...
/**
 * @brief - the user decides how the constructed phrase will be output.
 *
 * @param out - an &ostream object passed by the user.
 */
void Carton::Display(std::ostream &out) const
{
       out << setprecision(1) << fixed << "Carton: " << shipping_weight_ << " lbs. " << shipping_length_
           << " x " << width_ << " x " << height_ << endl;
}